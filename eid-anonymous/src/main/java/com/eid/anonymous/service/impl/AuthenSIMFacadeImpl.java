...
import com.eid.anonymous.service.AuthenSIMFacade;
...
@Slf4j
@Service
public class AuthenSIMFacadeImpl implements AuthenSIMFacade {
    ...
    // eID SIM 签名验签认证回调业务处理服务
    @Override
    public Response<JSONObject> eIDSIMCallBackHandle(JSONObject jsonObject) {
        log.info("IDSO回调IDSP-eID-SIM签名验签接口，方法名：eIDSIMCallBackHandle，回调参数JSON格式：{}",jsonObject);
        ...
        try {
            ...
            // 获取app信息
            CompanyAppEntity companyAppEntity = authenticationManager.findByApId(companyAuthenticationEntity.getApId());
...
//                apkeyfactor = companyAppEntity.getApKeyFactor();// 正式环境下的apkeyfactor，用于回调AP签名
            ...
            log.info("eID SIM 签名验签认证回调业务处理服务，本次认证结果：{}",flag);
...
//            String returnParam = appendReturnAPParam(appeidcode,apKeyFactor,companyAuthenticationEntity.getApId(),jsonObject);// 拼接回调参数
            JSONObject returnParam = appendReturnAPParam(appeidcode,apkeyfactor,companyAuthenticationEntity.getApId(),jsonObject);// 拼接回调参数
...
//                response.setResult(false);
    ...
    // 拼接IDSP回调AP的参数
    private JSONObject appendReturnAPParam(String appeidcode, String apKeyFactor, String apId, JSONObject jsonObject){
        ...
        log.info("IDSP回调AP签名数据：{}",jsonObject2);
...
//        hashMap.put("sign_type","HMAC_SHA1");
    ...
    private String userIdInfoAES(JSONObject jsonObject, String apKeyFactor, String apId){

        Response<String> resultAppKey = encryptionMachineFacade.getAppkey(apId,apKeyFactor);
        return encrypt(jsonObject.toString(),resultAppKey.getResult());
...
//            return new AESBase64().encode(encrypted);
    ...
    // 回调AP
    public String httpRequestAP(String url,Object param)
    {
        ...
        {
            // 设置请求参数 （此处json）
//            log.info("IDSO回调AP的报文，测试是否带斜杠：{}",param.toString());
//            String data = param.toString().replaceAll("\"[{]","{").replaceAll("[}]\"","}").replaceAll("\\\\","");
//            log.info("IDSO回调AP的报文，测试是否带斜杠：{}",data);
