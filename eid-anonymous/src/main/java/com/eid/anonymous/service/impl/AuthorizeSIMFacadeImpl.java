...
import com.eid.anonymous.service.AuthorizeSIMFacade;
import com.eid.anonymous.util.HttpClientUtil;
...
@Slf4j
@Service
public class AuthorizeSIMFacadeImpl implements AuthorizeSIMFacade {
...
//    {
//        String resultData = HttpClientUtil.httpclientRequest(HttpClientUtil.JSON_TYPE,"https://mztapp.fujian.gov.cn:20093/EidServer/jaxrs/eid/SimAuthCallback","");
////        String resultData = HttpClientUtil.httpclientRequest(HttpClientUtil.JSON_TYPE,"https://120.35.30.143:20093/EidServer/jaxrs/eid/SimAuthCallback","");
//        System.out.println(""+resultData);
//    }

    // eID SIM 身份授权认证回调业务处理服务
    @Override
    public Response<JSONObject> eIDSIMCallBackHandle(JSONObject jsonObject) {
        log.info("IDSO回调IDSP-eID-SIM身份授权接口，方法名：eIDSIMCallBackHandle，回调参数JSON格式：{}",jsonObject);
        Response<JSONObject> response = new Response<>();
        try {

            // 获取认证记录信息
            String resultDetail = jsonObject.getString("result_detail");// 获取响应详细结果码
            String bizSequenceId = jsonObject.getString("biz_sequence_id");// 获取认证流水号
...
//            String apkeyfactor = apkey;// 用于回调AP签名
            ...
            log.info("eID SIM 身份授权认证回调业务处理服务，本次认证结果：{}",flag);

            if(flag){// 认证成功处理

                // 解密userInfo(获取appKey,示例appkey：820948D2EF54CACD28722751D4031C7E)

                String userInfo = encryptUserInfo(jsonObject,apkey);// IDSO返回的原始userInfo解密后数据
                log.info("eID SIM 身份授权认证回调业务处理服务，解密userInfo结果：{}",userInfo);
                // 获取appeidcode
                String appeidcode = JSONObject.parseObject(userInfo).getString("appeidcode");
                log.info("SIM eID 身份授权返回的appeidcode:{}",appeidcode);

                // 获取返回到AP的身份信息
                resultUserInfo = getfInfo(bizSequenceId,userInfo,appeidcode);
                log.info("SIM-eID身份授权返回AP的身份信息:{}",resultUserInfo);

                // 在userInfo中拼接手机号码
                // 。。。。。。

                // 修改认证记录状态，更新IDSO返回的原始userInfo解密后的身份信息
                authenticationManager.success(companyAuthenticationEntity.getId(),resultDetail, ResCode.EID_0000000.getDesc(),appeidcode,userInfo);
            }
            else// 认证失败处理
            {
            ...
            // 拼接回调AP报文
            String returnParam = appendReturnAPParam(resultUserInfo,apkey,apId,jsonObject,apkeyfactor);// 拼接回调参数
            // 获取回调地址
...
//            String resultData = httpRequestAP(returnUrl,returnParam);
            // 兼容https
            String resultData = HttpClientUtil.httpclientRequest(HttpClientUtil.JSON_TYPE,returnUrl,returnParam);
            log.info("IDSP回调AP，AP响应数据：{}",resultData);

            // 验证回调AP结果并拼接响应IDSO的数据
            if(null != resultData && !"".equals(resultData) && resultData.contains("received") && resultData.contains("true")){
                // 拼接响应IDSO的数据
                log.info("响应IDSO----------------------------------------");
...
//                response.setResult(false);
        ...
        log.info("eID SIM 身份授权认证回调业务处理服务，处理结果数据：result:{};", response);
        return response;
    }

    // 获取AP是否要返回身份信息
    private String getfInfo(String bizSequenceId, String userInfo, String appeidcode){
        String fInfo = "1";
        JSONObject jsonObject1 = new JSONObject();
        try
        {
            fInfo = jedisCluster.get(bizSequenceId);
            log.info("SIM eID 身份授权是否需要返回AP身份信息状态：{}",fInfo);
            log.info("IDSO回调给IDSP的user_info：{}",userInfo);

            // 不返回身份信息
            jsonObject1.put("appeidcode", appeidcode);

            // 返回身份信息
            if(Integer.valueOf(fInfo)==1){
                JSONObject userInfoJson = JSONObject.parseObject(userInfo);
                jsonObject1.put("name",userInfoJson.getString("name"));
                jsonObject1.put("idnum",userInfoJson.getString("idnum"));
            }

        } catch (Exception e) {
            log.info("SIM-eID身份授权认证，认证完成后IDSP回调AP，获取AP是否要返回身份信息失败，bizSequenceId:" + bizSequenceId);
        }
    ...
    // 解密userInfo
    private String encryptUserInfo(JSONObject jsonObject, String appKey){
        ...
        log.info("eID SIM 身份授权认证回调业务处理服务，解密请求参数,security_factor:{};encrypt_facotr:{};appkey:{};user_id_info:{}",security_factor,encrypt_facotr,appKey,user_id_info);
        String encryptUserIdInfo = idsoCallBackFacade.doEncrypt3DesECBPKCS5(appKey,user_id_info,encrypt_facotr);
        log.info("eID SIM 身份授权认证回调业务处理服务，解密结果user_info:{}",encryptUserIdInfo);

        return encryptUserIdInfo;

    }

    // 拼接IDSP回调AP的参数
    private String appendReturnAPParam(String userInfo, String apkey, String apId, JSONObject jsonObject, String apkeyfactor){
...
//        JSONObject jsonObject1 = new JSONObject();
//                jsonObject1.put("appeidcode","xxxxxx");
//                jsonObject1.put("name","周俊峰");
//                jsonObject1.put("idnum","xxxxxxxx");
//        userInfo = jsonObject1.toJSONString();
        userInfo = Strings.isNullOrEmpty(userInfo) ? "" : userIdInfoAES(JSONObject.parseObject(userInfo),apkey,apId);//需要使用AES加密，秘钥为APPKEY
...
//            return new AESBase64().encode(encrypted);
    ...
    private String backAPSign(String signType, String key, String apId, Map<String, String> strToSign,String signFactor) {
        ...
        log.info("IDSP回调AP的待签原文:{}",preSignStr);
