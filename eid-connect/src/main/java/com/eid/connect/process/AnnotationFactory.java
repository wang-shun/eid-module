...
public class AnnotationFactory implements ApplicationContextAware {
    ...
    public <T> T getBizImpl(Class<T> T, BizType bizType) {
        try {
            List<Object> classes = getImplBeans(InterfaceImpl.class);
            for (Object object : classes) {

                InterfaceImpl impl = object.getClass().getAnnotation(InterfaceImpl.class);
                BizType[] bizTypes = impl.value();
                for (BizType at : bizTypes) {
                    if (at.equals(bizType)) {
                        return (T) object;
                    }
                }
            }
