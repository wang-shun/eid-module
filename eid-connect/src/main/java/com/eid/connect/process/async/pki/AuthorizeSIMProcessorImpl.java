 ...
 */
@Slf4j
@Component("pkiAsyncAuthorizeSIMProcessorImpl")
@InterfaceImpl(value = {BizType.AUTHORIZE_SIM_ASYNC})
public class AuthorizeSIMProcessorImpl extends SendProcessor {
    ...
    // eID SIM 身份识别异步请求IDSO
    @Override
    public EidBaseResult send(EidBaseParam eidBaseParam) {
        log.info("eID SIM 身份授权异步认证发送请求到IDSO，eID SIM 身份授权异步认证发送请求到IDSO，方法名：send，请求IDSO原始参数:{};", eidBaseParam);
//        EidPkiRealNameParam eidPkiRealNameParam = (EidPkiRealNameParam) eidBaseParam;
        ...
        log.info("eID SIM 身份授权异步认证发送请求到IDSO，方法名：send，请求IDSO原始参数:{};", eidPkiRealNameParam);
...
//        parameters.setBizType(EBizType.AUTHEN_SIM_ASYNC);//签名验签
        parameters.setBizType(EBizType.AUTHORIZE_SIM_ASYNC);//身份授权
        ...
        parameters.setReturnUrl(returnAuthorizeSIMUrl);
...
//        String appkey = eidBaseParam.getAppKey();
        ...
        log.info("SIM eID请求IDSO参数加密，appid:{}",eidPkiRealNameParam.getAppId());
        log.info("SIM eID请求IDSO参数加密，dataToSign:{}",eidPkiRealNameParam.getDataToSign());
        log.info("SIM eID请求IDSO参数加密，appkey:{}",appkey);
        log.info("SIM eID请求IDSO参数加密，encryptFactor:{}",parameters.getEncryptFactor());
        String signDataToSign = doEncrypt3DesECBPKCS5(appkey,eidPkiRealNameParam.getDataToSign(),parameters.getEncryptFactor());
        log.info("IDSP使用IDSO提供的加密算法对dataToSign进行加密的结果：{}",signDataToSign);
        parameters.setDataToSign(signDataToSign);
...
//        parameters.setEidSn(eidPkiRealNameParam.getEidSn());
        ...
        log.info("eID SIM 身份授权异步认证请求IDSO地址:{}",opSIMAddress);
        log.info("eID SIM 身份授权异步认证请求IDSO参数(未签名):{}",parameters);
...
//            String requestIDSOUrl = opSIMAddress+"/idsoserver/authen/async/"+asId;
            // 身份授权接口
            String requestIDSOUrl = opSIMAddress+"/idsoserver/authorize/identity/async/"+asId;
            log.info("eID SIM 身份授权异步认证请求IDSO完整路径：{}",requestIDSOUrl);
            PkiRNVSIMService service = new PkiRNVSIMService(new SHmacSha1Service(asKey), requestIDSOUrl); // sync request
            JSONObject result = service.doRequestAsyn(parameters);// 异步认证返回（{"received":"true"}）
            log.info("eID SIM 异步身份授权认证返回数据:"+result);
            // IDSO联调代码 end
...
//            result.put("received","true");
            ...
            // 将是否需要返回身份信息放入缓存
            jedisCluster.set(eidPkiRealNameParam.getBizSequenceId(),String.valueOf(eidPkiRealNameParam.getInfo()));
            jedisCluster.expire(eidPkiRealNameParam.getBizSequenceId(),60 * 30);//30分钟有效时间
