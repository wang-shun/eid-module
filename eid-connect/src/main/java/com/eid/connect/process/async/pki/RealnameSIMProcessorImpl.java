...
import org.aiav.astoopsdk.service.eidservice.biz.pki.PkiRNVService;
 ...
 */
@Slf4j
@Component("pkiAsyncRealnameSIMProcessorImpl")
@InterfaceImpl(value = {BizType.IDENTITY_SIM_ASYNC})
public class RealnameSIMProcessorImpl extends SendProcessor {
    ...
    public EidBaseResult send(EidBaseParam eidBaseParam) {
        log.info("eID SIM 身份识别异步认证发送请求到IDSO，方法名：send，请求IDSO原始参数:{};", eidBaseParam);
...
//        parameters.setBizType(EBizType.AUTHEN_SIM_ASYNC);//签名验签
        ...
        parameters.setReturnUrl(returnRealnameSIMUrl);
...
//        parameters.setEidSn(eidPkiRealNameParam.getEidSn());
        ...
        log.info("eID SIM 身份识别异步认证请求IDSO地址:{}",opSIMAddress);
        log.info("eID SIM 身份识别异步认证请求IDSO参数(未签名):{}",parameters);
